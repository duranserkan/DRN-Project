...
/**
 * Props for the HelloReact mounted island component.
 *
...
export const HelloReactComponent = ({ title, versions, onReady, onCardClick }: HelloReactProps) => {
    ...
    const lastInvokedOnReady = useRef<typeof onReady>(undefined);

    // Notify the host environment that the mounted React island is fully rendered.
    // Unlike hydrated islands (Astro/Fresh), mounted islands render entirely client-side
    // into server-rendered Razor Pages. No SSR or hydration step involved.
    // onReady payload handler via island.update()
    useEffect(() => {
        if (onReady && lastInvokedOnReady.current !== onReady) {
            onReady();
            lastInvokedOnReady.current = onReady;
        }
    }, [onReady]);
        ...
        <div className="hello-react">
            <div className="hello-react-hero">
                ...
                <ul className="hello-react-stack">
                    ...
                    <li><strong>React {v.react}</strong> mounted islands in isolated Shadow DOM (client-rendered, not hydrated)</li>
