...
- Source-Known input format: operations `Parse`/`Validate` and entity/repository GUID helpers accept `SourceKnownEntityIdFormat? format = null` from SharedKernel.Domain. Omission follows `UseSecureSourceKnownIds` (true → Secure, false → Plain); explicit formats override configuration. Auto retains detection; conversions accept both forms. Record `ValidateId`/`Validate` checks stored metadata only: null/Auto preserves prior validation, explicit Secure/Plain checks the flag. Undefined values throw even for null input/empty repository batches; supported formats retain null results. Consumers must rebuild for changed signatures.
