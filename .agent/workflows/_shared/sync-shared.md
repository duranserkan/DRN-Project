...
| `directional-source-change` | Directional source change with unchanged target (including security gates, confirmation checks, lockout protection, defensive validation, login/auth enhancements, or architectural pattern updates in domain-bound files) | Adapt source intent to target domain entities and namespaces; preserve target identity and domain invariants. Partial syncs of shared features or rules without mutating target domain entities are valid `directional-source-change` operations. |
...
Mandate (Non-Homogenization & Source Pattern Adaptation): `/sync` MUST NOT merge or homogenize target domain entities or erase target-specific domain invariants between endpoints. Synchronization targets consistency of common infrastructure, patterns, security controls, and conventions. Partial synchronization of shared features, authentication flows (e.g., login flow improvements), or rules is encouraged when it improves security, rules, or feature parity without altering target domain entity bounds or forcing endpoints to become identical. When directional source changes introduce security controls, confirmation gates, lockout protection, or architectural patterns inside domain-bound files (e.g. PageModels, Controllers), `/sync` MUST NOT skip the file under `equal-or-variant`. Instead, it MUST classify the architectural/security improvement under `directional-source-change`, adapt the source pattern to target domain entities (e.g., substituting target user types and namespaces), and propose the adapted change.
