...
| **`/answer auto`** | Present `DEVELOP-*.md`, pause, display confirmation prompt, **require explicit "yes"** before invoking `/develop`. |
| **User requests changes** | Minor edits → update `DEVELOP-*.md` directly. Significant scope change → return to `/clarify`. |
...
> [!WARNING]
> **`/clarify auto` chain**: `/clarify auto` triggers `/answer auto`, which triggers `/develop` after confirmation. Only safe for **low-risk, well-understood tasks**. The confirmation gate is mandatory and cannot be skipped.