...
*   **Strict Entity-Bound ID Generation**: Constrained `ISourceKnownIdUtils.Next<TEntity>()` and `SourceKnownIdUtils.Generate<TEntity>(byte appId, byte appInstanceId)` to `where TEntity : SourceKnownEntity` (changing `Generate<TEntity>` from `where TEntity : class`), and bound dynamic `Next(SourceKnownEntity)`, `Next(Type, ...)`, and `Generate(Type, ...)` invocations strictly to `SourceKnownEntity` subclasses. This eliminates ambient `_nexusAppId` fallback and ensures partition enforcement across all generator paths. Generic ID generation for non-entity or arbitrary class types (such as `Generate<object>(...)`) is prohibited and no longer supported. Migration guidance: consumers generating Source-Known IDs must derive their domain types from `SourceKnownEntity` (and annotate concrete classes with `[EntityType<TApp>]` for attribute-derived partition dispatch with `Next<TEntity>()`, or supply explicit partition and instance values with `SourceKnownIdUtils.Generate<TEntity>(appId, appInstanceId)`).
*   **ISourceKnownIdUtils Contract Expansion**: Added `Next(SourceKnownEntity entity)` and `Next(Type entityType, byte appId, byte appInstanceId, DateTimeOffset? epoch)` as required members on `ISourceKnownIdUtils`. Custom implementations must implement both overloads, typically by wrapping an existing `ISourceKnownIdUtils` implementation or using `SourceKnownIdUtils.Generate(Type, ...)` for the explicit-partition overload.
*   **ISourceKnownEntityIdUtils Contract Expansion**: Added parameterless `Generate<TEntity>()`, `GeneratePlain<TEntity>()`, and `GenerateSecure<TEntity>()` as required members on `ISourceKnownEntityIdUtils`. Custom implementations must implement all three overloads, typically by forwarding to the corresponding `ISourceKnownIdUtils.Next<TEntity>()` and conversion operations.
...
*   **Instance & Dynamic Next ID Generation**: Added `Next(SourceKnownEntity entity)` and `Next(Type entityType, byte appId, byte appInstanceId, DateTimeOffset? epoch)` overloads on `ISourceKnownIdUtils` and `SourceKnownIdUtils`, along with `Generate(Type entityType, ...)` and `Warmup(ICollection<Type> entityTypes)` static helpers. Steady-state lookups use direct typed delegates compiled via `MethodInfo.CreateDelegate` and cached in an immutable `FrozenDictionary<Type, Func<...>>` snapshot swapped atomically during `Warmup`, with thread-safe `ConcurrentDictionary` dynamic fallback for ad-hoc / mock types to eliminate runtime reflection overhead and bucket lock contention on entity insertion hot paths.
*   **MethodUtils Fast Invocation API**: Upgraded `MethodUtils` with `System.Reflection.MethodInvoker` runtime native stubs, zero-allocation specialized overloads (0, 1, 2, 3 arguments, `Span<object?>`, and `params object?[]`), unified zero-allocation cached method discovery (`FindMethod`), public uncached discovery (`FindMethodUncached`), and unified high-level invocation helpers (`InvokeMethod` and `InvokeStaticMethod`). Eliminated duplicate cache dictionaries, redundant reflection scanners, and legacy aliases (`InvokeGenericMethod`, `InvokeStaticGenericMethod`, `FindGenericMethod`, `FindNonGenericMethod`). Low-level invokers (`InvokeFast`) and helper structs are encapsulated internally to provide a clean, fast-by-default public API.
