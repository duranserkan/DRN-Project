...
*   **ISourceKnownIdUtils Contract Expansion**: Added `Next(Type entityType)` and `Next(Type entityType, byte appId, byte appInstanceId, DateTimeOffset? epoch)` to `ISourceKnownIdUtils`. Interface defaults can be inherited by consumers and implementations, with explicit implementation required only when custom behavior is needed.
