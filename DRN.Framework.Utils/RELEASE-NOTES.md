...
*   **Non-Generic Next(Type) & Generate(Type) ID Generation**: Added `Next(Type entityType)` and `Next(Type entityType, byte appId, byte appInstanceId, DateTimeOffset? epoch)` overloads on `ISourceKnownIdUtils` and `SourceKnownIdUtils`, along with `Generate(Type entityType, ...)` and `Warmup(IEnumerable<Type> entityTypes)` static helpers. Dynamic type generation paths use direct typed delegates created via `MethodInfo.CreateDelegate` and cached in `ConcurrentDictionary<Type, Func<...>>` to eliminate runtime `MethodInfo.Invoke` reflection overhead on steady state. `Warmup` pre-binds and caches delegates for domain entity types at startup to avoid first-call binding latency.
