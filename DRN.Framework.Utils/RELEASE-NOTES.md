...
*   **ISourceKnownIdUtils Contract Expansion**: Added `Next(Type entityType)` and `Next(Type entityType, byte appId, byte appInstanceId, DateTimeOffset? epoch)` to `ISourceKnownIdUtils`. Custom or mock implementations of `ISourceKnownIdUtils` must implement these members or delegate to `SourceKnownIdUtils.Next` / `SourceKnownIdUtils.Generate`.
