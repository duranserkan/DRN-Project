...
*   **Unified Authentication Claims**: `ConfigureAuthenticationClaims()` supplies explicit Identity defaults and aliases for subject/name/email/roles plus the exact MFA marker. Hosting derives Identity claim options from it; scoped users, policies, exemptions, assurance, enrollment, rate limiting, and renewal use the same configuration. Identity factories supply canonical types/native metadata directly, without universal claim rewriting. `DrnSignInManager` requires an unambiguous subject and retains factory metadata; custom factories must honor the claim contract. Provider handlers and remote account flows remain separate integrations.
*   **Identity Lifecycle Integration**: Added overridable `ConfigureIdentityRenewal` and explicitly registered `DrnSignInManager<TUser>` for custom local MFA issuance and complete evidence preservation during `RefreshSignInAsync`. Sample and Nexus retain their existing Identity journeys with this registration. Existing custom managers must integrate explicitly.
...
*   **Configured Subjects**: Explicit mappings accept only selected types and aliases. Conflicting subjects/issuers reject account binding, including rate-limit partition selection. Native authorization requires an integrated handler or Identity factory/manager using the same mapping.
*   **MFA Renewal**: Cookie stamp, explicit cookie refresh, and bearer refresh preserve original authenticated evidence and claim metadata, rejecting account mismatch and discarding factory-generated authentication evidence. Split marker/time evidence cannot gain recent-MFA assurance. Bearer refresh validates the security stamp and rejects restricted credentials. See [renewal and assurance](README.md#renewal-and-assurance).
