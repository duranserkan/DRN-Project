...
*   **Partition-Scoped Entity Type Validation**: Runtime startup validation now discovers and validates both `DbSet` and non-`DbSet` domain entities across host assemblies, treating `(EntityType, AppId)` as the uniqueness key, allowing different application partitions to reuse the same entity type byte consistently with SharedKernel analyzers, while enforcing that single `DbContext` instances do not combine multiple production `AppId` partitions, and verifying that configured `NexusAppSettings.AppId` matches either the DbContext's direct domain partition or a recognized primary partition registered in the host.
