...
*   **PostgreSQL Migration Provider Lifetime**: `PostgresContext` now serializes migration-provider construction with migration execution and disposes that temporary provider before the real `WebApplicationFactory` host starts, preventing independently owned migration resources from overlapping the test host.
