...
## 28. Security Gates Must Fail Closed on Missing Analysis Artifacts

### Context

The shared CodeQL verification action reads SARIF generated by `github/codeql-action/analyze` before master and release workflows can pass.

### Problem

Treating a missing SARIF directory, empty SARIF directory, malformed SARIF, or fragile parser pipeline as success turns scanner failure or path drift into a passing security gate. SARIF result fields can also be optional, and the effective level may come from `result.level` or the referenced rule's `defaultConfiguration.level`.

### Fix Applied

The verifier now requires at least one parseable `.sarif` or `.sarif.json` file, validates that each file has a non-empty `runs` array, resolves the effective SARIF level from result override to rule default to warning fallback, runs `jq` under strict shell handling, and fails on any non-`note`/`none` result. Workflows pass `steps.codeql-analysis.outputs.sarif-output` from the producer action instead of duplicating the output path.
