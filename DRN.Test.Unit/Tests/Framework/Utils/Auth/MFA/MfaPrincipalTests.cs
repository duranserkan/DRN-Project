...
public class MfaPrincipalTests
{
    ...
    public void Completion_Should_Use_Authenticated_Unrestricted_Evidence(
        string scenario, ClaimsPrincipal principal, MfaClaimConfig config, bool expected)
    {
    ...
    {
        var config = MfaClaimConfig.AspNetIdentity;
        ...
        var customConfig = new MfaClaimConfig("acr", "urn:drn:test:mfa");
        var customIdentity = new ClaimsIdentity([new Claim(customConfig.ClaimType, customConfig.ClaimValue)], "Federated");
