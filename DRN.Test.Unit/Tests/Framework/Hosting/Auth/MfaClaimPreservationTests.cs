...
public class MfaClaimPreservationTests
{
        ...
        string claimType, string claimValue, string scenario, bool expected, string issuer)
    {
        ...
        MfaPrincipal.IsCompleted(renewed, config).Should().BeTrue();
        ...
        target.HasClaim("auth_time", "1700000000").Should().Be(expected || scenario == "untrusted-marker");
    ...
    public void Renewal_Should_Preserve_Marker_Provenance_Alongside_A_Marker_From_Another_Issuer(string claimType, string claimValue)
    {
        ...
        target.FindAll(claimType).Should().HaveCount(2);
    ...
    public void Renewal_Should_Preserve_Original_Time_And_Provenance_Without_Duplicates()
    {
        ...
        MfaClaimPreservation.Preserve(source, target, MfaClaimConfig.AspNetIdentity);
        MfaClaimPreservation.Preserve(source, target, MfaClaimConfig.AspNetIdentity);
    ...
    public void Renewal_Should_Not_Create_Time_When_Original_Is_Missing_Or_Invalid(string? value)
    {
        ...
        MfaClaimPreservation.Preserve(new ClaimsPrincipal(identity), target,
            new MfaClaimConfig("auth_time", string.IsNullOrWhiteSpace(value) ? "missing" : value));
    ...
    public void Renewal_Should_Reject_Time_From_Unbound_Identity(string subject, string issuer, bool authenticated)
    {
        ...
        MfaClaimPreservation.Preserve(new ClaimsPrincipal(identity), target, MfaClaimConfig.AspNetIdentity);
    ...
    public void Renewal_Should_Reject_Conflicting_Authenticated_Evidence(string value, string issuer, string originalIssuer, string subject)
    {
        ...
        MfaClaimPreservation.Preserve(new ClaimsPrincipal([identity, secondary]), target, MfaClaimConfig.AspNetIdentity);
    ...
    public void Renewal_Should_Ignore_Unauthenticated_Time_And_Reject_Ambiguous_Target()
    {
        ...
        MfaClaimPreservation.Preserve(source, anonymousTarget, MfaClaimConfig.AspNetIdentity);
