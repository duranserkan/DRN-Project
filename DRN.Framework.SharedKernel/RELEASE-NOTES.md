...
*   **Entity ID Format Contracts**: Added `SourceKnownEntityIdFormat` (`Secure`, `Plain`, `Auto`) to nullable/nonnullable operations `Parse`/`Validate` and domain/repository GUID helpers. Utils uses its configured secure/plain default when omitted; explicit Auto retains detection of both forms. `SourceKnownEntityId.ValidateId`/`Validate` accept an optional format check against stored metadata; omission/Auto preserves prior record validation and does not reauthenticate the GUID. Undefined formats throw, including on null inputs and empty repository batches. Ordinary calls remain valid; rebuild binary consumers and update custom operations/repository implementations and method-group bindings for the expanded contracts.

*   **Explicit Expected Identity**: Replace `SourceKnownEntityId.Validate(byte)` and byte-only domain `GetEntityId` calls with generic entity overloads or `new EntityTypeId(entityType, expectedAppId)`. Validation now checks both components. `ValidateId()` and the domain GUID helper's boolean option remain integrity-only.
