%% Custom Pandoc template for arXiv preprint using wlpeerj.cls
%% Usage: pandoc --template=template-arxiv.tex paper.md ...
%%
%% Derived from template-peerj.tex with arXiv-specific adjustments:
%%   - Line numbers removed (arXiv rejects submissions with line numbers)
%%   - Fonts specified by TeX filename (arXiv has no macOS system fonts)
%%
...
precedents for SKID's topology fields and SKEID's metadata layout.
...
strong pseudorandom-permutation assumption (Bellare \& Rogaway, 2000,
Theorem 2). SKEID incorporates an embedded MAC, and Secure SKEID follows
this principle by encrypting the structured representation and verifying
it after decryption.

For external use, Khuong (2022) describes using AES to convert a
...
\item
  ...
  the generator.
\item
  \textbf{Zero-expansion plaintext-to-encrypted identifier conversion}
  that combines embedded metadata, a truncated MAC, and collision-guard
...
(Davis, Peabody \& Leach, 2024, sec. 2.1). These schemes illustrate
different choices in identifier size, ordering, timestamp precision, and
generator metadata. We include CUID2 as CUID's successor, UUID V4 and V7
as standardized random and time-ordered baselines, and database
sequences as the compact storage baseline. This selection covers
relevant design trade-offs without reproducing the RFC's full survey.
...
draft specification (Kılıç, 2026b). The collision-guard scenario that
follows is hypothetical and is not triggered by this vector.
...
key in the test vector in RFC 3394 (Schaad \& Housley, 2002, sec. 4.3).
The MAC and AES keys shown below are derived from this master key by the
...
DRN.Framework (Kılıç, 2026c), organized across four NuGet packages.
...
\item
  ...
  (Kılıç, 2026c). However, AES-256 encryption replaces the plaintext
  Version and Variant marker bytes with pseudorandom ciphertext. The
  encrypted output is not guaranteed to preserve the version and variant
  bits required by RFC 9562 Sections 4.1 and 5.8. Strict RFC 9562
  validators may therefore reject a Secure SKEID. Systems requiring
  strict RFC 9562 compliance at the validator level should accept Secure
  SKEIDs as opaque strings or 128-bit integers.
\end{enumerate}
...
\item
  ...
  (Kılıç, 2026b) is available in the archived DRN-Project v0.10.0
...
\item
  ...
  unnecessary inspection (Davis, Peabody \& Leach, 2024, sec. 6.12). It
  also defines Nil and Max UUIDs with all 128 bits set to zero or one,
  respectively, and version values 9--15 remain reserved for future
  definitions rather than being permanently excluded from the UUID
  version space (Davis, Peabody \& Leach, 2024, secs. 4.2, 5.9, and
  5.10). These provisions distinguish general UUID handling from
  validation of a particular version, but do not establish RFC
  conformance for arbitrary ciphertext or require parsers to accept it.
  The current results suggest that preserving UUIDv8 bits may also be
  unnecessary on other contemporary platforms. Whether this holds for
  most practical deployments remains to be established. Documenting
  compatibility across platform and library versions is left as future
  work. Where the complete application path accepts Secure SKEIDs,
  techniques such as cycle walking or specialized Feistel constructions
  (Khuong, 2022) may be unnecessary solely for UUID compatibility.
\end{enumerate}
...
\bibitem[\citeproctext]{ref-bellare2000}
Bellare M, Rogaway P. 2000. {Encode-then-encipher encryption: How to
exploit nonces or redundancy in plaintexts for efficient cryptography}.
In: Okamoto T ed. \emph{Advances in cryptology -- ASIACRYPT 2000}.
Lecture notes in computer science. Berlin, Heidelberg: Springer,
317--330. DOI:
\href{https://doi.org/10.1007/3-540-44448-3_24}{10.1007/3-540-44448-3\_24}.

\bibitem[\citeproctext]{ref-bonnetain2019}
Bonnetain X, Naya-Plasencia M, Schrottenloher A. 2019. {Quantum Security
Analysis of AES}. \emph{IACR Trans. Symmetric Cryptol.} 2019:55--93.
DOI:
...
IEEE. 2020. \emph{{IEEE Std 1588-2019: IEEE Standard for a Precision
...
Kılıç DS. 2026c. {DRN-Project: Distributed Reliable .Net}. DOI:
\href{https://doi.org/10.5281/zenodo.22863243}{10.5281/zenodo.22863243}.
...
\bibitem[\citeproctext]{ref-skid-preprint}
Kılıç DS. 2026a. {Source Known Identifiers: A Three-Tier Identity System
for Distributed Applications}. DOI:
\href{https://doi.org/10.48550/arXiv.2604.00151}{10.48550/arXiv.2604.00151}.
...
Intelligence in World War II}. \emph{J. Am. Stat. Assoc.} 42:72--91.
DOI:
\href{https://doi.org/10.1080/01621459.1947.10501915}{10.1080/01621459.1947.10501915}.

\bibitem[\citeproctext]{ref-rfc3394}
Schaad J, Housley R. 2002. \emph{{Advanced Encryption Standard (AES) Key
Wrap Algorithm}}. IETF. DOI:
\href{https://doi.org/10.17487/RFC3394}{10.17487/RFC3394}.
